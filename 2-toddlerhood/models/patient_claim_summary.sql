...
select 
    ...
    d.npi,
    (select max(specialty) from {{ source('DBT_MATURITY_EHR_DATA', 'doc_specialties') }}  ds where ds.doc_id = d.id) as spec_name,
    ...
    c.firstbill
from 
    {{ source('DBT_MATURITY_EHR_DATA', 'patients') }}  p
    join {{ source('DBT_MATURITY_BILLING_DATA', 'claims') }}  c
        on p.id = c.pat_id
    left join (
        select 
            cl.claim_id, 
        ...
        from {{ source('DBT_MATURITY_BILLING_DATA', 'claim_line') }} cl
    ...
    left join {{ source('DBT_MATURITY_EHR_DATA', 'doctors') }}  d
        on c.doc_id = d.id 
